        ...
        <v-list-item v-for="song in this.playlist" :key="song.title">
          <v-list-item-icon>
            <v-btn
              fab
              x-small
              @click="setCurrentSong(song, true)"
              elevation="0"
            ...
            <v-btn
              ...
              @click="pause"
              elevation="0"
              v-if="song == currentSong && isPlaying == true"
            >
            ...
            <v-btn
              ...
              @click="play"
              elevation="0"
              v-if="song == currentSong && isPlaying == false"
            >
              <v-icon color="black"> mdi-play </v-icon>
            </v-btn>
          </v-list-item-icon>
  ...
  methods: {
    ...
    pause: function () {
      ...
      try {
        ...
        _wavesurfer.backend.setState("paused"); 
        console.error(error);
      }
    },
    onFinish: function () {
    ...
    nextSong: function (wasPlaying, onFinish) {
      ...
      let currentIndex = this.songOrder.indexOf(
        this.songOrder.find(
          (index) => index == this.playlist.indexOf(this.currentSong)
        )
      );
      let newIndex = null;
      if (this.repeat == "once") {
